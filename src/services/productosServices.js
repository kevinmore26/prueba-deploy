...
const URL = `https://llegoshopback.herokuapp.com/gestion/productos/`;
...
const obtenerProductos = async () => {
  ...
  try {
    ...
    // equivalente al .then
    let { data } = await axios.get(`https://llegoshopback.herokuapp.com/gestion/productos/`);
...
const obtenerProductosFiltro = async (busqueda) => {
  ...
  try {
    let { data } = await axios.get(
      `https://llegoshopback.herokuapp.com/gestion/producto-filtro?disponible=true&nombre=${busqueda}`
    );
    console.log(data.content);
    return data.content;
  } catch {
...
const obtenerProductoPorId = async (id) => {
  try {
    let { data } = await axios.get(
      `https://llegoshopback.herokuapp.com/gestion/producto/${id}`
    );
    console.log(data);
    return data.content;
  } catch {
